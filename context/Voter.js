...
export const VotingProvider = ({ children }) => {
  ...
  const [electionArray, setElectionarray] = useState(pushelection)
  ...
  const uploadToIPFSElection = async file => {
    if (file) {
      try {
        const formData = new FormData()
        formData.append('file', file)
  ...
  const createelection = async (formInput, router) => {
    try {
      const { name, description, startdate, enddate } = formInput
      ...
      const data = JSON.stringify({ name, description, startdate, enddate})
      //const added = await client(data);
      const formData = new FormData()
      formData.append('file', data)
      ...
      const election = await contract.createElection(name, description, startdate, enddate, url)
      election.wait()
  ...
  const setCandidate = async (candidateform, fileUrl, router) => {
    try {
      const { name, address, age, election } = candidateform

      if (!name || !address || !age || !election)
        //if (!name || !address || !position)
        //return console.log(name, address, position, fileUrl)
        return console.log('Input data is missing');

        console.log(name, address, age,election, fileUrl);
      ...
      const data = JSON.stringify({ name, address, election, image: fileUrl, age })
      //const added = await client(data);
      const formData = new FormData()
      formData.append('file', data)
      ...
      const candidate = await contract.setCandidate(address, age, name, fileUrl, election, ipfs)
      candidate.wait()
